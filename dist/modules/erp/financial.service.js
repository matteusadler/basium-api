...
let FinancialService = class FinancialService {
    ...
    async markPaid(id, companyId, paymentData) {
        const entry = await this.prisma.financialEntry.findFirst({
            where: { id, companyId },
        });
        if (!entry) {
    ...
    async generateOwnerTransfer(companyId, contractId, month) {
        ...
        if (!rentEntry) {
            throw new common_1.NotFoundException('Aluguel do mês ainda não foi pago');
        }
    ...
    async getByCategory(companyId, filters) {
        const where = { companyId, status: 'PAID' };
        if (filters?.startDate || filters?.endDate) {
            where.dueDate = {};
            if (filters.startDate)
                where.dueDate.gte = new Date(filters.startDate);
