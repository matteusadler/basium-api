...
export class DashboardService {
  ...
  async getStats(companyId: string) {
    ...
    const totalRevenue = wonLeadsWithValue.reduce(
      (sum, lead) => sum + (lead.closedValue || 0), 
      0
    )
    ...
    const revenueThisMonth = wonLeadsThisMonthWithValue.reduce(
      (sum, lead) => sum + (lead.closedValue || 0), 
      0
    )
  ...
  async getLeadsChart(companyId: string, period: string = '6months') {
    const now = new Date()
    let startDate: Date
    ...
    // Agrupar por período
    const chartData: Record<string, { novos: number; ganhos: number; perdidos: number }> = {}
    ...
    leads.forEach(lead => {
      ...
      chartData[key].novos++
      if (lead.status === 'WON') chartData[key].ganhos++
      if (lead.status === 'LOST') chartData[key].perdidos++
    })
  ...
  async getTodayTasks(companyId: string, userId?: string) {
  ...
  async getLeadsByTemperature(companyId: string) {
